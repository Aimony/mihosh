...
func TestResolveConnectionsMouseHit_FindsSiteAndConnectionTargets(t *testing.T) {
	...
	for y := 0; y < state.Height; y++ {
		for x := 0; x < state.Width; x++ {
			hit := ResolveConnectionsMouseHit(state, x, y)
			if hit.Target == ConnectionsMouseTargetSiteTest && hit.Index == 2 {
				siteFound = true
			}
			if hit.Target == ConnectionsMouseTargetConnection && hit.Index == 0 {
				connFound = true
			}
		}
	}
...
func TestResolveConnectionsMouseHit_WithCharts_AlignsFirstVisibleRow(t *testing.T) {
	...
	hit := ResolveConnectionsMouseHit(state, 0, firstRowY)
	if hit.Target != ConnectionsMouseTargetConnection || hit.Index != 0 {
