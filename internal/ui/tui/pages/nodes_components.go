...
// NodesMouseTarget 表示 nodes 页面鼠标命中的列表组件
type NodesMouseTarget int

const (
	NodesMouseTargetNone NodesMouseTarget = iota
	NodesMouseTargetGroup
	NodesMouseTargetProxy
)

// NodesMouseHit 是 nodes 页面鼠标命中结果
type NodesMouseHit struct {
	Target NodesMouseTarget
	Index  int
}
...
// ResolveNodesMouseHit 根据 pageContent 内的 Y 坐标定位命中的策略组/节点行。
func ResolveNodesMouseHit(state NodesPageState, pageY int) NodesMouseHit {
	...
	if len(state.GroupNames) > 0 {
		...
		if pageY >= groupDataStart && pageY < groupDataEnd {
			return NodesMouseHit{
				Target: NodesMouseTargetGroup,
	...
	if len(state.CurrentProxies) > 0 {
		...
		if pageY >= proxyDataStart && pageY < proxyDataEnd {
			return NodesMouseHit{
				Target: NodesMouseTargetProxy,
				Index:  proxyWindow.ScrollTop + (pageY - proxyDataStart),
			}
		}
	}

	return NodesMouseHit{Target: NodesMouseTargetNone, Index: -1}
}
...
func renderGroupListComponent(state NodesPageState, groupMaxLines int) string {
	if len(state.GroupNames) == 0 {
		return "  正在加载..."
	}
...
func renderProxyListComponent(state NodesPageState, proxyMaxLines int) string {
	if len(state.CurrentProxies) == 0 {
		if state.FilterText != "" {
			return "  无搜索结果"
		}
